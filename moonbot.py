...
def executeJob(sc, app=PyCryptoBot(), trading_data=pd.DataFrame()):
    ...
    if len(df_last) > 0:
        ...
        if macdgtsignalco and ema12gtema26 and obv_pc < 400 and obv_pc > -400 and obv_pc_delta1 < 400 and obv_pc_delta1 > -400 and elder_ray_bull > elder_ray_bull_delta1 and elder_ray_bear > elder_ray_bear_delta1 and last_action != 'BUY':
            action = 'BUY'
        ...
        if last_buy > 0 and last_action == 'BUY':
            ...
            if margin > 1 and app.sellUpperPcnt() != None and fib_high > fib_low and fib_high <= float(price) and macdltsignal:
                action = 'SELL'
                last_action = 'BUY'
            ...
            # profit bank at sell at fibonacci band
            if margin <= 1 and app.sellUpperPcnt() != None and fib_high > fib_low and fib_high <= float(price):
                action = 'SELL'
                last_action = 'BUY'
                log_text = '! Profit Bank Triggered (Fibonacci Band: ' + str(fib_high) + ')'
                print (log_text, "\n")
            ...
            if fib_high > float(price) and macdltsignal and macdgtsignalco_delta1 == False:
                action = 'SELL'
                last_action = 'BUY'
