...
def executeJob(sc, app=PyCryptoBot(), trading_data=pd.DataFrame()):
    ...
    if len(df_last) > 0:
        ...
        elder_ray_bull_delta1 = float(df_last['elder_ray_bull_delta1'].values[0])
        elder_ray_bear_delta1 = float(df_last['elder_ray_bear_delta1'].values[0])
        elder_ray_bull_delta2 = float(df_last['elder_ray_bull_delta2'].values[0])
        elder_ray_bear_delta2 = float(df_last['elder_ray_bear_delta2'].values[0])
        obv_pc_delta1 = float(df_last['obv_pc_delta1'].values[0])
        close_delta1 = float(df_last['close_delta1'].values[0])
        close_delta2 = float(df_last['close_delta2'].values[0])
        ...
        if last_buy > 0 and last_action == 'BUY':
            ...
            # do not sell on loss (off by default)
        ...
        if (last_df_index != current_df_index):
            ...
            if app.isVerbose() == 0:
                if last_action != '':
                    output_text = current_df_index + ' | ' + app.getMarket() + bullbeartext + ' | ' + str(
                        app.getGranularity()) + ' | ' + price_text + ' | ' + ema_co_prefix + ema_text + ema_co_suffix + ' | ' + macd_co_prefix + macd_text + macd_co_suffix + ' | ' + obv_prefix + obv_text + obv_suffix + ' | ' + eri_text + ' | ' + action + ' | Last Action: ' + last_action
                else:
                    output_text = current_df_index + ' | ' + app.getMarket() + bullbeartext + ' | ' + str(
                        app.getGranularity()) + ' | ' + price_text + ' | ' + ema_co_prefix + ema_text + ema_co_suffix + ' | ' + macd_co_prefix + macd_text + macd_co_suffix + ' | ' + obv_prefix + obv_text + obv_suffix + ' | ' + eri_text + ' | ' + action + ' '
            ...
            if action == 'BUY':
                ...
                if app.isLive() == 1:
                    ...
                    try:
                        resp = app.marketBuy(app.getMarket(), float(account.getBalance(app.getQuoteCurrency())) / 2)
                        logging.info(resp)
                    except:
                        action = 'WAIT'
                        last_action = 'SELL'
                        print('! ERROR: Likely insufficient funds to place BUY order. Reverting to last action = SELL',
                              "\n")
                    ...
                    if app.isVerbose() == 0:
                        ...
                        if (sell_price != 0):
                            buy_sell_margin_no_fees = str(
                                app.truncate((((sell_price - last_buy_price) / sell_price) * 100), 2)) + '%'
                        else:
                        ...
                        if (sell_price != 0):
                            buy_sell_margin_fees = str(
                                app.truncate((((sell_price - last_buy_price_minus_fees) / sell_price) * 100), 2)) + '%'
                        else:
                            buy_sell_margin_fees = '0%'

                        logging.info(current_df_index + ' | ' + app.getMarket() + ' ' + str(
                            app.getGranularity()) + ' | SELL | ' + str(sell_price) + ' | BUY | ' + str(
                            last_buy_price) + ' | DIFF | ' + str(buy_sell_diff) + ' | MARGIN NO FEES | ' + str(
                            buy_sell_margin_no_fees) + ' | MARGIN FEES | ' + str(buy_sell_margin_fees))
                        print("\n", current_df_index, '|', app.getMarket(), str(app.getGranularity()), '| SELL |',
                              str(sell_price), '| BUY |', str(last_buy_price), '| DIFF |', str(buy_sell_diff),
                              '| MARGIN NO FEES |', str(buy_sell_margin_no_fees), '| MARGIN FEES |',
...
try:
    # initialise logging
    logging.basicConfig(filename='pycryptobot.log', format='%(asctime)s - %(levelname)s: %(message)s',
